...
Se non riesci ad accedere al contenuto, prova a immaginare una struttura logica basata su titolo e descrizione.`}];try{const r=(await E(t,y.OUTLINE_EXTRACTION,o,1500)).choices[0].message.content.trim(),a=ce(r);return{url:e,title:n,outline:a}}catch(s){throw console.error("Error extracting outline:",s),s}}function ce(t){return t.split(`
`).filter(i=>{const o=i.toLowerCase();return!ie.some(s=>o.includes(s.toLowerCase()))}).join(`
...
Rispondi SOLO con il nuovo subheading migliorato, senza spiegazioni aggiuntive.`}];try{return(await E(this.openrouterApiKey,y.SEMANTIC_ANALYSIS,o,100)).choices[0].message.content.trim()}catch(s){throw console.error("Error generating AI suggestion:",s),s}}getCurrentOutlineText(){return this.headings.map(e=>(e.level==="H1"?"# ":e.level==="H2"?"## ":"### ")+e.text).join(`
`)}getUpdatedStatistics(){const e=this.headings.map(o=>o.score).filter(o=>o!==null),n=K(e),i=C(n);return{totalHeadings:this.headings.length,overallScore:n,overallLevel:i,scoreDistribution:this.calculateScoreDistribution()}}}const f=new fe;function ye(t,e){const n=document.getElementById("phase3Section");n&&(n.classList.remove("hidden"),S(n),n.scrollIntoView({behavior:"smooth"}),n.dataset.outlineText=t,n.dataset.topic=e,Ee())}function Ee(){const t=document.getElementById("startSemanticAnalysisBtn"),e=document.getElementById("copyFinalOutlineBtn");t&&t.addEventListener("click",be),e&&e.addEventListener("click",Be)}async function be(){var t;try{const e=document.getElementById("geminiApiKey").value.trim(),n=document.getElementById("openrouterApiKey").value.trim(),i=document.getElementById("phase3Section"),o=i.dataset.outlineText,s=i.dataset.topic;if(!e){m("API key di Google Gemini richiesta per l'analisi semantica.");return}if(!n){m("API key di OpenRouter richiesta per i suggerimenti AI.");return}if(!o||!s){m("Nessuna outline trovata. Completa prima la Fase 2.");return}k("Analizzando coerenza semantica..."),f.setApiKeys(e,n);const r=(t=window.outlineGenerator)==null?void 0:t.getSelectedData();r!=null&&r.semanticAnalysis&&f.setSemanticTerms(r.semanticAnalysis);const a=await f.analyzeOutlineSemanticCoherence(s,o);ve(a),h(),v("Analisi semantica completata con successo!")}catch(e){h(),m(`Errore durante l'analisi semantica: ${e.message}`),console.error("Semantic analysis error:",e)}}function ve(t){const e=document.getElementById("semanticAnalysisResults");e&&(e.classList.remove("hidden"),S(e),Q(t.statistics),Y(t.statistics.scoreDistribution),Se(t.headings))}function Q(t){const e=document.getElementById("overallScore");if(e){const n=q(t.overallLevel);e.className=`px-3 py-1 rounded-full text-sm font-medium ${n}`,e.textContent=`Punteggio Complessivo: ${b(t.overallScore)}`}}function Y(t){const e={excellentCount:document.getElementById("excellentCount"),goodCount:document.getElementById("goodCount"),fairCount:document.getElementById("fairCount"),poorCount:document.getElementById("poorCount")};e.excellentCount&&(e.excellentCount.textContent=t.excellent),e.goodCount&&(e.goodCount.textContent=t.good),e.fairCount&&(e.fairCount.textContent=t.fair),e.poorCount&&(e.poorCount.textContent=t.poor)}function Se(t){const e=document.getElementById("interactiveOutlineEditor");e&&(e.innerHTML="",t.forEach((n,i)=>{const o=xe(n);e.appendChild(o)}))}function xe(t,e){const n=document.createElement("div");n.className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600",n.dataset.headingId=t.id;const i=q(t.scoreLevel);return n.innerHTML=`
    ...
    `,Ce(n,t),n}function Ce(t,e){const n=t.querySelector(".ai-suggest-btn"),i=t.querySelector(".edit-btn"),o=t.querySelector(".cancel-edit-btn"),s=t.querySelector(".confirm-edit-btn"),r=t.querySelector("input");n&&n.addEventListener("click",()=>ke(e.id)),i&&i.addEventListener("click",()=>we(t)),o&&o.addEventListener("click",()=>H(t)),s&&s.addEventListener("click",()=>_(t,e.id)),r&&r.addEventListener("keydown",a=>{a.key==="Enter"?_(t,e.id):a.key==="Escape"&&H(t)})}function we(t){const e=t.querySelector(".heading-display"),n=t.querySelector(".heading-edit");if(e&&n){e.classList.add("hidden"),n.classList.remove("hidden");const i=n.querySelector("input");i&&(i.focus(),i.select())}}function H(t){const e=t.querySelector(".heading-display"),n=t.querySelector(".heading-edit");e&&n&&(e.classList.remove("hidden"),n.classList.add("hidden"))}async function _(t,e){const n=t.querySelector("input");if(!n)return;const i=n.value.trim();if(!i){m("Il testo del subheading non può essere vuoto.");return}try{k("Ricalcolando punteggio...");const o=await f.recalculateHeadingScore(e,i);Ie(t,i,o),H(t),$e(),h(),Ae(t,o)}catch(o){h(),m(`Errore durante il ricalcolo: ${o.message}`),console.error("Recalculation error:",o)}}function Ie(t,e,n){const i=t.querySelector(".heading-display"),o=t.querySelector(".px-2.py-1");if(i&&(i.textContent=e),o){const s=q(n.scoreLevel);o.className=`px-2 py-1 text-xs rounded-full ${s}`,o.textContent=b(n.score)}}function Ae(t,e){const n=t.querySelector(".score-change-indicator");if(!n||e.improvement===null)return;const i=e.improvement,o=i>0,s=o?`+${b(Math.abs(i))}`:`-${b(Math.abs(i))}`,r=o?"text-green-600 dark:text-green-400":"text-red-600 dark:text-red-400";n.innerHTML=`
        <div class="text-xs ${r}">
            ${o?"📈":"📉"} ${s}
        </div>
