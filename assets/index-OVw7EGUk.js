(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function n(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(s){if(s.ep)return;s.ep=!0;const o=n(s);fetch(s.href,o)}})();function N(){const t=localStorage.getItem("openrouter_api_key"),e=document.getElementById("openrouterApiKey");t&&e&&(e.value=t)}function M(t,e){e.trim()&&localStorage.setItem(`${t}_api_key`,e)}function R(){N(),console.log("✅ AI Outline Generator inizializzato")}function P(){const t=localStorage.getItem("dark_mode")==="true",e=document.getElementById("darkModeToggle"),n=document.getElementById("darkModeIcon");t&&(document.documentElement.classList.add("dark"),$(n,!0)),e&&e.addEventListener("click",()=>{const i=document.documentElement.classList.toggle("dark");localStorage.setItem("dark_mode",i.toString()),$(n,i)})}function $(t,e){t&&(t.innerHTML=e?'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"></path>':'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>')}const H="https://openrouter.ai/api/v1",A=3,k=1e3,g={SERP_SEARCH:"perplexity/sonar",OUTLINE_EXTRACTION:"openai/gpt-4o-mini",SEMANTIC_ANALYSIS:"anthropic/claude-3-5-sonnet-20241022",ASPECT_GENERATION:"anthropic/claude-3-5-sonnet-20241022",OUTLINE_GENERATION:"anthropic/claude-3-5-sonnet-20241022",FALLBACK_MODEL:"openai/gpt-3.5-turbo"},_=["conclusione","conclusioni","chi siamo","chiama ora","contattaci","contatti","richiedi preventivo","preventivo","offerta","promozione","sconto","privacy policy","termini di servizio","cookie policy","disclaimer","footer","sidebar","menu","navigazione","breadcrumb","condividi","social","newsletter","iscriviti","seguici","testimonial","recensioni cliente","clienti","portfolio","servizi correlati","prodotti correlati","articoli correlati","biografia autore","autore","about us","team","staff","faq generiche","domande frequenti generiche","help","supporto","login","registrati","account","profilo utente"];async function h(t,e,n,i=1e3){if(!t||!e||!n)throw new Error("API key, model, and messages are required");const s=`${H}/chat/completions`,o={model:e,messages:n,max_tokens:i,temperature:.7,top_p:1,frequency_penalty:0,presence_penalty:0};return await y(s,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`,"HTTP-Referer":window.location.origin,"X-Title":"AI Outline Generator"},body:JSON.stringify(o)})}async function D(t,e){const n=[{role:"user",content:`Cerca su Google.it i primi 10 risultati per l'argomento "${e}". 
...
`).filter(i=>{const s=i.toLowerCase();return!_.some(o=>s.includes(o.toLowerCase()))}).join(`
`)}async function W(t,e){const n=[{role:"system",content:"Sei un esperto linguista e semantologo. Il tuo compito è creare un'analisi semantica completa e dettagliata di un argomento specifico."},{role:"user",content:`Think this step by step.
Topic=${e}
...
}`}];try{const o=(await h(t,g.SEMANTIC_ANALYSIS,n,4e3)).choices[0].message.content.match(/\{[\s\S]*\}/);if(o)return JSON.parse(o[0]);throw new Error("Invalid response format")}catch(i){throw console.error("Error generating semantic analysis:",i),i}}async function F(t,e,n){const i=[{role:"system",content:"Sei un esperto content strategist. Il tuo compito è analizzare le outline dei competitor e identificare tutti gli aspetti che dovrebbero essere trattati per creare un contenuto completo e competitivo su un argomento."},{role:"user",content:`Analizza queste outline dei competitor per l'argomento "${e}" e identifica tutti gli aspetti che dovrebbero essere trattati:
...
}`}];try{const r=(await h(t,g.ASPECT_GENERATION,i,3e3)).choices[0].message.content.match(/\{[\s\S]*\}/);if(r)return JSON.parse(r[0]);throw new Error("Invalid response format")}catch(s){throw console.error("Error generating aspects to treat:",s),s}}async function y(t,e,n=0){var i;try{const s=await fetch(t,e);if(!s.ok){const r=await s.json().catch(()=>({}));if(s.status===429&&n<A){const c=k*Math.pow(2,n);return console.warn(`Rate limited. Retrying in ${c}ms...`),await x(c),y(t,e,n+1)}if(s.status>=500&&n<A){const c=k*Math.pow(2,n);return console.warn(`Server error ${s.status}. Retrying in ${c}ms...`),await x(c),y(t,e,n+1)}const a=((i=r.error)==null?void 0:i.message)||r.message||"Unknown error",l=s.status===400?`Bad Request - Check API key and model availability. ${a}`:`HTTP ${s.status} - ${a}`;throw new Error(`OpenRouter API Error ${s.status}: ${l}`)}const o=await s.json();if(!o.choices||!o.choices[0]||!o.choices[0].message)throw new Error("Invalid response format from OpenRouter API");return o}catch(s){if(s.name==="TypeError"&&s.message.includes("fetch"))throw new Error("Errore di connessione. Verifica la tua connessione internet.");if(n<A&&(s.message.includes("network")||s.message.includes("timeout")||s.message.includes("fetch"))){const o=k*Math.pow(2,n);return console.warn(`Network error. Retrying in ${o}ms...`),await x(o),y(t,e,n+1)}throw s}}function x(t){return new Promise(e=>setTimeout(e,t))}async function J(t,e,n,i=!0){const s=[{role:"system",content:"Sei un esperto content strategist e SEO specialist. Il tuo compito è creare un'outline completa e semanticamente ottimizzata per un contenuto che deve coprire al 100% tutti gli aspetti di un argomento."},{role:"user",content:`Crea un'outline completa per l'argomento "${e}" utilizzando i seguenti dati:
