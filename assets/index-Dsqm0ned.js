(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();function O(){const t=localStorage.getItem("openrouter_api_key"),e=document.getElementById("openrouterApiKey");t&&e&&(e.value=t)}function $(t,e){e.trim()&&localStorage.setItem(`${t}_api_key`,e)}function N(){O(),console.log("✅ AI Outline Generator inizializzato")}function M(){const t=localStorage.getItem("dark_mode")==="true",e=document.getElementById("darkModeToggle"),s=document.getElementById("darkModeIcon");t&&(document.documentElement.classList.add("dark"),L(s,!0)),e&&e.addEventListener("click",()=>{const o=document.documentElement.classList.toggle("dark");localStorage.setItem("dark_mode",o.toString()),L(s,o)})}function L(t,e){t&&(t.innerHTML=e?'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"></path>':'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>')}const R="https://openrouter.ai/api/v1",w=3,x=1e3,g={SERP_SEARCH:"perplexity/llama-3.1-sonar-large-128k-online",OUTLINE_EXTRACTION:"openai/gpt-4o-mini",SEMANTIC_ANALYSIS:"anthropic/claude-3-5-sonnet-20241022",ASPECT_GENERATION:"anthropic/claude-3-5-sonnet-20241022",OUTLINE_GENERATION:"anthropic/claude-3-5-sonnet-20241022"},_=["conclusione","conclusioni","chi siamo","chiama ora","contattaci","contatti","richiedi preventivo","preventivo","offerta","promozione","sconto","privacy policy","termini di servizio","cookie policy","disclaimer","footer","sidebar","menu","navigazione","breadcrumb","condividi","social","newsletter","iscriviti","seguici","testimonial","recensioni cliente","clienti","portfolio","servizi correlati","prodotti correlati","articoli correlati","biografia autore","autore","about us","team","staff","faq generiche","domande frequenti generiche","help","supporto","login","registrati","account","profilo utente"];async function h(t,e,s,o=1e3){if(!t||!e||!s)throw new Error("API key, model, and messages are required");const n=`${R}/chat/completions`,i={model:e,messages:s,max_tokens:o,temperature:.7,top_p:1,frequency_penalty:0,presence_penalty:0};return await E(n,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`,"HTTP-Referer":window.location.origin,"X-Title":"AI Outline Generator"},body:JSON.stringify(i)})}async function P(t,e){const s=[{role:"user",content:`Cerca online i primi 10 risultati per l'argomento "${e}". 
...
Se non riesci ad accedere al contenuto, prova a immaginare una struttura logica basata su titolo e descrizione.`}];try{const r=(await h(t,g.OUTLINE_EXTRACTION,n,1500)).choices[0].message.content.trim(),a=H(r);return{url:e,title:s,outline:a}}catch(i){throw console.error("Error extracting outline:",i),i}}function H(t){return t.split(`
`).filter(o=>{const n=o.toLowerCase();return!_.some(i=>n.includes(i.toLowerCase()))}).join(`
`)}async function j(t,e){const s=[{role:"system",content:"Sei un esperto linguista e semantologo. Il tuo compito è creare un'analisi semantica completa e dettagliata di un argomento specifico."},{role:"user",content:`Think this step by step.
Topic=${e}
...
Rispondi in formato JSON:
{
    ...
    `,document.body.appendChild(s),setTimeout(()=>{s.parentElement&&s.remove()},5e3)}function W(t){const e={success:"bg-green-100 text-green-800 border border-green-200 dark:bg-green-900 dark:text-green-200 dark:border-green-800",error:"bg-red-100 text-red-800 border border-red-200 dark:bg-red-900 dark:text-red-200 dark:border-red-800",warning:"bg-yellow-100 text-yellow-800 border border-yellow-200 dark:bg-yellow-900 dark:text-yellow-200 dark:border-yellow-800",info:"bg-blue-100 text-blue-800 border border-blue-200 dark:bg-blue-900 dark:text-blue-200 dark:border-blue-800"};return e[t]||e.info}function J(t){const e={success:`<svg class="h-5 w-5 text-green-400" fill="currentColor" viewBox="0 0 20 20">
        ...
        </svg>`};return e[t]||e.info}function K(t,e=300){t.style.opacity="0",t.style.transition=`opacity ${e}ms ease-in-out`,setTimeout(()=>{t.style.opacity="1"},10)}function I(t){X(t.competitorOutlines),Y(t.semanticAnalysis),ee(t.aspectsToTreat),y(),Q()}function Q(){const t=document.getElementById("resultsSection");t&&(t.classList.remove("hidden"),K(t))}function X(t){const e=document.getElementById("competitorSection"),s=document.getElementById("competitorOutlines");!e||!s||!t||(s.innerHTML="",t.forEach((o,n)=>{const i=document.createElement("div");i.className="competitor-outline-item border border-gray-200 dark:border-gray-600 rounded-lg p-4 mb-4";const r=document.createElement("div");r.className="flex items-center justify-between mb-3";const a=document.createElement("div");a.className="flex-1";const d=document.createElement("h3");d.className="font-semibold text-gray-900 dark:text-white text-sm",d.textContent=o.title||`Competitor ${n+1}`;const c=document.createElement("a");c.className="text-xs text-blue-600 dark:text-blue-400 hover:underline",c.href=o.url,c.target="_blank",c.textContent=o.url,a.appendChild(d),a.appendChild(c);const u=document.createElement("input");u.type="checkbox",u.id=`competitor-${n}`,u.checked=l.results.selectedElements.competitors.has(n),u.className="ml-4",u.addEventListener("change",()=>{l.toggleCompetitorSelection(n),y()}),r.appendChild(a),r.appendChild(u);const p=document.createElement("div");p.className="outline-content bg-gray-50 dark:bg-gray-700 p-3 rounded text-sm font-mono whitespace-pre-line",p.textContent=o.outline||"Nessuna outline estratta",i.appendChild(r),i.appendChild(p),s.appendChild(i)}))}function Y(t){const e=document.getElementById("semanticSection"),s=document.getElementById("semanticAnalysis");!e||!s||!t||(s.innerHTML="",m("Entità Principali",t.salientEntities,"salient",s),m("Sinonimi",t.synonyms,"synonym",s),t.categories&&Object.keys(t.categories).forEach(o=>{const n=t.categories[o];Z(o,n,s)}),m("Relazioni Tutto-Parte",t.partWholeRelationships,"partWholeRelationships",s),m("Collocazioni Comuni",t.commonCollocations,"commonCollocations",s),m("Affermazioni Correlate",t.relatedStatements,"relatedStatements",s),m("Domande Correlate",t.relatedQuestions,"relatedQuestions",s),m("Errori Comuni",t.commonErrors,"commonErrors",s),m("Dibattiti Attuali",t.currentDebates,"currentDebates",s))}function m(t,e,s,o){if(!e||!Array.isArray(e)||e.length===0)return;const n=document.createElement("div");n.className="semantic-section mb-6";const i=document.createElement("h4");i.className="font-semibold text-gray-900 dark:text-white mb-3",i.textContent=t;const r=document.createElement("div");r.className="flex flex-wrap gap-2",e.forEach((a,d)=>{const c=document.createElement("span");c.className="semantic-item px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-sm cursor-pointer border border-transparent hover:border-blue-300",c.textContent=a,c.dataset.elementId=`${s}_${d}`,l.results.selectedElements.semanticElements.has(`${s}_${d}`)&&(c.classList.add("selected"),c.classList.add("bg-green-100","dark:bg-green-900","text-green-800","dark:text-green-200"),c.classList.remove("bg-blue-100","dark:bg-blue-900","text-blue-800","dark:text-blue-200")),c.addEventListener("click",()=>{l.toggleSemanticElementSelection(`${s}_${d}`),se(c),y()}),r.appendChild(c)}),n.appendChild(i),n.appendChild(r),o.appendChild(n)}function Z(t,e,s){if(!e||!e.entities)return;const o=document.createElement("div");o.className="semantic-category mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg";const n=document.createElement("h4");n.className="font-semibold text-gray-900 dark:text-white mb-3",n.textContent=`Categoria: ${t}`,o.appendChild(n),m("Entità",e.entities,`category_${t}_entity`,o),e.examples&&m("Esempi",e.examples,`category_${t}_example`,o),e.attributes&&m("Attributi",e.attributes,`category_${t}_attribute`,o),s.appendChild(o)}function ee(t){const e=document.getElementById("aspectsSection"),s=document.getElementById("aspectsList");!e||!s||!t||(s.innerHTML="",t.aspectsByCategory&&Object.keys(t.aspectsByCategory).forEach(o=>{const n=t.aspectsByCategory[o],i=document.createElement("div");i.className="aspects-category mb-6";const r=document.createElement("h4");r.className="font-semibold text-gray-900 dark:text-white mb-3",r.textContent=o,i.appendChild(r),n.forEach((a,d)=>{const c=document.createElement("div");c.className="aspect-item flex items-start space-x-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg mb-2";const u=document.createElement("input");u.type="checkbox",u.id=`aspect-${o}-${d}`,u.checked=l.results.selectedElements.aspects.has(`${o}_${d}`),u.className="mt-1",u.addEventListener("change",()=>{l.toggleAspectSelection(`${o}_${d}`),y()});const p=document.createElement("div");p.className="flex-1";const b=document.createElement("h5");b.className="font-medium text-gray-900 dark:text-white",b.textContent=a.aspect;const v=document.createElement("p");v.className="text-sm text-gray-600 dark:text-gray-400 mt-1",v.textContent=a.description;const f=document.createElement("div");f.className="flex items-center space-x-4 mt-2";const C=document.createElement("span");if(C.className=`px-2 py-1 text-xs rounded-full ${te(a.priority)}`,C.textContent=a.priority,f.appendChild(C),a.foundInCompetitors&&a.foundInCompetitors.length>0){const S=document.createElement("span");S.className="text-xs text-gray-500 dark:text-gray-400",S.textContent=`Trovato in ${a.foundInCompetitors.length} competitor`,f.appendChild(S)}p.appendChild(b),p.appendChild(v),p.appendChild(f),c.appendChild(u),c.appendChild(p),i.appendChild(c)}),s.appendChild(i)}))}function te(t){const e={high:"bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200",medium:"bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200",low:"bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200"};return e[t]||e.medium}function se(t){t.classList.contains("selected")?(t.classList.remove("selected"),t.classList.remove("bg-green-100","dark:bg-green-900","text-green-800","dark:text-green-200"),t.classList.add("bg-blue-100","dark:bg-blue-900","text-blue-800","dark:text-blue-200")):(t.classList.add("selected"),t.classList.add("bg-green-100","dark:bg-green-900","text-green-800","dark:text-green-200"),t.classList.remove("bg-blue-100","dark:bg-blue-900","text-blue-800","dark:text-blue-200"))}function y(){const t=l.getStatistics(),e=document.getElementById("competitorCount");e&&(e.textContent=`${t.selectedCompetitorCount}/${t.competitorCount}`);const s=document.getElementById("semanticCount");s&&(s.textContent=`${t.selectedSemanticElements}/${t.totalSemanticElements}`);const o=document.getElementById("aspectCount");o&&(o.textContent=`${t.selectedAspects}/${t.totalAspects}`);const n=document.getElementById("statsCard");n&&t.competitorCount>0&&n.classList.remove("hidden");const i=document.getElementById("proceedBtn");if(i){const r=t.selectedCompetitorCount>0||t.selectedSemanticElements>0||t.selectedAspects>0;i.disabled=!r}}function ne(){const t=document.getElementById("selectAllBtn"),e=document.getElementById("deselectAllBtn"),s=document.getElementById("proceedBtn");t&&t.addEventListener("click",()=>{l.selectAllElements(),I(l.results)}),e&&e.addEventListener("click",()=>{l.deselectAllElements(),I(l.results)}),s&&s.addEventListener("click",()=>{const o=l.getSelectedData();console.log("Selected data for phase 2:",o),alert("Fase 2 non ancora implementata. I dati selezionati sono stati salvati nella console.")})}function oe(){ie(),ae(),ce(),de(),ne()}function ie(){const t=document.getElementById("toggleOpenrouterApiKey");t&&t.addEventListener("click",()=>{re("openrouterApiKey","openrouterEyeIcon")})}function re(t,e){const s=document.getElementById(t),o=document.getElementById(e);if(!s||!o)return;const n=s.type==="password";s.type=n?"text":"password",o.innerHTML=n?'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21" />':'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />'}function ae(){["openrouterApiKey","topicInput"].forEach(e=>{const s=document.getElementById(e);s&&s.addEventListener("input",()=>{if(e.includes("ApiKey")){const o=e.replace("ApiKey","");$(o,s.value)}setTimeout(()=>B(),0)})})}function ce(){const t=document.getElementById("generateBtn");t&&t.addEventListener("click",le)}async function le(){try{const t=document.getElementById("openrouterApiKey").value.trim(),e=document.getElementById("topicInput").value.trim();if(!B())return;if(F("Verifica API key..."),!await U(t)){A(),T("API key OpenRouter non valida. Verifica e riprova.");return}const o=await l.generateCompleteOutline(t,e);I(o),A(),V("Generazione completata con successo!")}catch(t){A(),T(`Errore durante la generazione: ${t.message}`),console.error("Generation error:",t)}}function de(){l.setProgressCallback((t,e)=>{ue(t,e)}),l.setStatusCallback(t=>{me(t)})}function ue(t,e){const s=document.getElementById("progressSection"),o=document.getElementById(`step${t}`);if(s&&s.classList.remove("hidden"),o){const n=o.querySelector(".w-4.h-4.rounded-full"),i=o.querySelector("span");n&&(n.className="w-4 h-4 rounded-full bg-blue-500"),i&&(i.textContent=e,i.className="text-sm font-medium text-blue-600 dark:text-blue-400");for(let r=1;r<t;r++){const a=document.getElementById(`step${r}`);if(a){const d=a.querySelector(".w-4.h-4.rounded-full");d&&(d.className="w-4 h-4 rounded-full bg-green-500");const c=a.querySelector("span");c&&(c.className="text-sm text-green-600 dark:text-green-400")}}}}function me(t){const e=document.getElementById("statusMessage");e&&(e.textContent=t)}function B(){const t=document.getElementById("openrouterApiKey").value.trim(),e=document.getElementById("topicInput").value.trim(),s=document.getElementById("generateBtn"),o=document.getElementById("generateHelp");let n=!0,i="";return t?e?e.length<3&&(n=!1,i="L'argomento deve essere di almeno 3 caratteri"):(n=!1,i="Inserisci un argomento/topic da analizzare"):(n=!1,i="Inserisci la tua API key di OpenRouter"),s&&(s.disabled=!n),o&&(i?(o.textContent=i,o.classList.remove("hidden")):o.classList.add("hidden")),n}document.addEventListener("DOMContentLoaded",()=>{N(),M(),oe(),document.body.classList.add("loaded")});
