(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();function z(){const t=localStorage.getItem("openrouter_api_key"),e=document.getElementById("openrouterApiKey");t&&e&&(e.value=t)}function O(t,e){e.trim()&&localStorage.setItem(`${t}_api_key`,e)}function N(){z(),console.log("✅ AI Outline Generator inizializzato")}function M(){const t=localStorage.getItem("dark_mode")==="true",e=document.getElementById("darkModeToggle"),s=document.getElementById("darkModeIcon");t&&(document.documentElement.classList.add("dark"),L(s,!0)),e&&e.addEventListener("click",()=>{const o=document.documentElement.classList.toggle("dark");localStorage.setItem("dark_mode",o.toString()),L(s,o)})}function L(t,e){t&&(t.innerHTML=e?'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"></path>':'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>')}const R="https://openrouter.ai/api/v1",w=3,x=1e3,h={SERP_SEARCH:"perplexity/sonar",OUTLINE_EXTRACTION:"openai/gpt-4o-mini",SEMANTIC_ANALYSIS:"anthropic/claude-3-5-sonnet-20241022",ASPECT_GENERATION:"anthropic/claude-3-5-sonnet-20241022",OUTLINE_GENERATION:"anthropic/claude-3-5-sonnet-20241022",FALLBACK_MODEL:"openai/gpt-3.5-turbo"},_=["conclusione","conclusioni","chi siamo","chiama ora","contattaci","contatti","richiedi preventivo","preventivo","offerta","promozione","sconto","privacy policy","termini di servizio","cookie policy","disclaimer","footer","sidebar","menu","navigazione","breadcrumb","condividi","social","newsletter","iscriviti","seguici","testimonial","recensioni cliente","clienti","portfolio","servizi correlati","prodotti correlati","articoli correlati","biografia autore","autore","about us","team","staff","faq generiche","domande frequenti generiche","help","supporto","login","registrati","account","profilo utente"];async function g(t,e,s,o=1e3){if(!t||!e||!s)throw new Error("API key, model, and messages are required");const n=`${R}/chat/completions`,i={model:e,messages:s,max_tokens:o,temperature:.7,top_p:1,frequency_penalty:0,presence_penalty:0};return await E(n,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`,"HTTP-Referer":window.location.origin,"X-Title":"AI Outline Generator"},body:JSON.stringify(i)})}async function P(t,e){const s=[{role:"user",content:`Search online for the top 10 results about "${e}". 
            ...
            }`}];try{const i=(await g(t,h.SERP_SEARCH,s,2e3)).choices[0].message.content.match(/\{[\s\S]*\}/);if(i)return JSON.parse(i[0]).results||[];throw new Error("Invalid response format from Perplexity")}catch(o){return console.warn("Perplexity search failed, using fallback with GPT-4o-mini:",o.message),await D(t,e)}}async function D(t,e){const s=[{role:"system",content:"You are a helpful assistant that generates realistic example URLs and content for competitor analysis based on a given topic."},{role:"user",content:`Generate 10 realistic competitor URLs and content descriptions for the topic "${e}". These should be typical pages that would rank well for this topic.
            ...
            }`}];try{const i=(await g(t,h.FALLBACK_MODEL,s,1500)).choices[0].message.content.match(/\{[\s\S]*\}/);if(i)return JSON.parse(i[0]).results||[];throw new Error("Invalid response format")}catch(o){return console.error("Error in fallback competitor search:",o),H(e)}}function H(t){return[{url:"https://example1.com/guide",title:`Complete Guide to ${t}`,description:`Comprehensive guide covering all aspects of ${t}`},{url:"https://blog.example2.com/article",title:`${t}: Everything You Need to Know`,description:`Detailed article explaining ${t} fundamentals`},{url:"https://example3.com/tutorial",title:`How to Master ${t}`,description:`Step-by-step tutorial for ${t}`},{url:"https://example4.com/tips",title:`Top 10 ${t} Tips`,description:`Expert tips and best practices for ${t}`},{url:"https://example5.com/advanced",title:`Advanced ${t} Strategies`,description:`Advanced techniques and strategies for ${t}`}]}async function j(t,e,s,o){const n=[{role:"system",content:"Sei un esperto nell'estrazione di outline da contenuti web. Il tuo compito è identificare la struttura gerarchica dei contenuti (H1, H2, H3) da una pagina web."},{role:"user",content:`Estrai l'outline strutturale dalla seguente pagina web:
...
Se non riesci ad accedere al contenuto, prova a immaginare una struttura logica basata su titolo e descrizione.`}];try{const r=(await g(t,h.OUTLINE_EXTRACTION,n,1500)).choices[0].message.content.trim(),a=G(r);return{url:e,title:s,outline:a}}catch(i){throw console.error("Error extracting outline:",i),i}}function G(t){return t.split(`
`).filter(o=>{const n=o.toLowerCase();return!_.some(i=>n.includes(i.toLowerCase()))}).join(`
`)}async function U(t,e){const s=[{role:"system",content:"Sei un esperto linguista e semantologo. Il tuo compito è creare un'analisi semantica completa e dettagliata di un argomento specifico."},{role:"user",content:`Think this step by step.
Topic=${e}
...
}`}];try{const i=(await g(t,h.SEMANTIC_ANALYSIS,s,4e3)).choices[0].message.content.match(/\{[\s\S]*\}/);if(i)return JSON.parse(i[0]);throw new Error("Invalid response format")}catch(o){throw console.error("Error generating semantic analysis:",o),o}}async function q(t,e,s){const o=[{role:"system",content:"Sei un esperto content strategist. Il tuo compito è analizzare le outline dei competitor e identificare tutti gli aspetti che dovrebbero essere trattati per creare un contenuto completo e competitivo su un argomento."},{role:"user",content:`Analizza queste outline dei competitor per l'argomento "${e}" e identifica tutti gli aspetti che dovrebbero essere trattati:
...
Rispondi in formato JSON:
{
    ...
    `,document.body.appendChild(s),setTimeout(()=>{s.parentElement&&s.remove()},5e3)}function K(t){const e={success:"bg-green-100 text-green-800 border border-green-200 dark:bg-green-900 dark:text-green-200 dark:border-green-800",error:"bg-red-100 text-red-800 border border-red-200 dark:bg-red-900 dark:text-red-200 dark:border-red-800",warning:"bg-yellow-100 text-yellow-800 border border-yellow-200 dark:bg-yellow-900 dark:text-yellow-200 dark:border-yellow-800",info:"bg-blue-100 text-blue-800 border border-blue-200 dark:bg-blue-900 dark:text-blue-200 dark:border-blue-800"};return e[t]||e.info}function Q(t){const e={success:`<svg class="h-5 w-5 text-green-400" fill="currentColor" viewBox="0 0 20 20">
        ...
        </svg>`};return e[t]||e.info}function Y(t,e=300){t.style.opacity="0",t.style.transition=`opacity ${e}ms ease-in-out`,setTimeout(()=>{t.style.opacity="1"},10)}function I(t){Z(t.competitorOutlines),ee(t.semanticAnalysis),se(t.aspectsToTreat),y(),X()}function X(){const t=document.getElementById("resultsSection");t&&(t.classList.remove("hidden"),Y(t))}function Z(t){const e=document.getElementById("competitorSection"),s=document.getElementById("competitorOutlines");!e||!s||!t||(s.innerHTML="",t.forEach((o,n)=>{const i=document.createElement("div");i.className="competitor-outline-item border border-gray-200 dark:border-gray-600 rounded-lg p-4 mb-4";const r=document.createElement("div");r.className="flex items-center justify-between mb-3";const a=document.createElement("div");a.className="flex-1";const l=document.createElement("h3");l.className="font-semibold text-gray-900 dark:text-white text-sm",l.textContent=o.title||`Competitor ${n+1}`;const c=document.createElement("a");c.className="text-xs text-blue-600 dark:text-blue-400 hover:underline",c.href=o.url,c.target="_blank",c.textContent=o.url,a.appendChild(l),a.appendChild(c);const u=document.createElement("input");u.type="checkbox",u.id=`competitor-${n}`,u.checked=d.results.selectedElements.competitors.has(n),u.className="ml-4",u.addEventListener("change",()=>{d.toggleCompetitorSelection(n),y()}),r.appendChild(a),r.appendChild(u);const p=document.createElement("div");p.className="outline-content bg-gray-50 dark:bg-gray-700 p-3 rounded text-sm font-mono whitespace-pre-line",p.textContent=o.outline||"Nessuna outline estratta",i.appendChild(r),i.appendChild(p),s.appendChild(i)}))}function ee(t){const e=document.getElementById("semanticSection"),s=document.getElementById("semanticAnalysis");!e||!s||!t||(s.innerHTML="",m("Entità Principali",t.salientEntities,"salient",s),m("Sinonimi",t.synonyms,"synonym",s),t.categories&&Object.keys(t.categories).forEach(o=>{const n=t.categories[o];te(o,n,s)}),m("Relazioni Tutto-Parte",t.partWholeRelationships,"partWholeRelationships",s),m("Collocazioni Comuni",t.commonCollocations,"commonCollocations",s),m("Affermazioni Correlate",t.relatedStatements,"relatedStatements",s),m("Domande Correlate",t.relatedQuestions,"relatedQuestions",s),m("Errori Comuni",t.commonErrors,"commonErrors",s),m("Dibattiti Attuali",t.currentDebates,"currentDebates",s))}function m(t,e,s,o){if(!e||!Array.isArray(e)||e.length===0)return;const n=document.createElement("div");n.className="semantic-section mb-6";const i=document.createElement("h4");i.className="font-semibold text-gray-900 dark:text-white mb-3",i.textContent=t;const r=document.createElement("div");r.className="flex flex-wrap gap-2",e.forEach((a,l)=>{const c=document.createElement("span");c.className="semantic-item px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-sm cursor-pointer border border-transparent hover:border-blue-300",c.textContent=a,c.dataset.elementId=`${s}_${l}`,d.results.selectedElements.semanticElements.has(`${s}_${l}`)&&(c.classList.add("selected"),c.classList.add("bg-green-100","dark:bg-green-900","text-green-800","dark:text-green-200"),c.classList.remove("bg-blue-100","dark:bg-blue-900","text-blue-800","dark:text-blue-200")),c.addEventListener("click",()=>{d.toggleSemanticElementSelection(`${s}_${l}`),oe(c),y()}),r.appendChild(c)}),n.appendChild(i),n.appendChild(r),o.appendChild(n)}function te(t,e,s){if(!e||!e.entities)return;const o=document.createElement("div");o.className="semantic-category mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg";const n=document.createElement("h4");n.className="font-semibold text-gray-900 dark:text-white mb-3",n.textContent=`Categoria: ${t}`,o.appendChild(n),m("Entità",e.entities,`category_${t}_entity`,o),e.examples&&m("Esempi",e.examples,`category_${t}_example`,o),e.attributes&&m("Attributi",e.attributes,`category_${t}_attribute`,o),s.appendChild(o)}function se(t){const e=document.getElementById("aspectsSection"),s=document.getElementById("aspectsList");!e||!s||!t||(s.innerHTML="",t.aspectsByCategory&&Object.keys(t.aspectsByCategory).forEach(o=>{const n=t.aspectsByCategory[o],i=document.createElement("div");i.className="aspects-category mb-6";const r=document.createElement("h4");r.className="font-semibold text-gray-900 dark:text-white mb-3",r.textContent=o,i.appendChild(r),n.forEach((a,l)=>{const c=document.createElement("div");c.className="aspect-item flex items-start space-x-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg mb-2";const u=document.createElement("input");u.type="checkbox",u.id=`aspect-${o}-${l}`,u.checked=d.results.selectedElements.aspects.has(`${o}_${l}`),u.className="mt-1",u.addEventListener("change",()=>{d.toggleAspectSelection(`${o}_${l}`),y()});const p=document.createElement("div");p.className="flex-1";const b=document.createElement("h5");b.className="font-medium text-gray-900 dark:text-white",b.textContent=a.aspect;const v=document.createElement("p");v.className="text-sm text-gray-600 dark:text-gray-400 mt-1",v.textContent=a.description;const f=document.createElement("div");f.className="flex items-center space-x-4 mt-2";const C=document.createElement("span");if(C.className=`px-2 py-1 text-xs rounded-full ${ne(a.priority)}`,C.textContent=a.priority,f.appendChild(C),a.foundInCompetitors&&a.foundInCompetitors.length>0){const S=document.createElement("span");S.className="text-xs text-gray-500 dark:text-gray-400",S.textContent=`Trovato in ${a.foundInCompetitors.length} competitor`,f.appendChild(S)}p.appendChild(b),p.appendChild(v),p.appendChild(f),c.appendChild(u),c.appendChild(p),i.appendChild(c)}),s.appendChild(i)}))}function ne(t){const e={high:"bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200",medium:"bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200",low:"bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200"};return e[t]||e.medium}function oe(t){t.classList.contains("selected")?(t.classList.remove("selected"),t.classList.remove("bg-green-100","dark:bg-green-900","text-green-800","dark:text-green-200"),t.classList.add("bg-blue-100","dark:bg-blue-900","text-blue-800","dark:text-blue-200")):(t.classList.add("selected"),t.classList.add("bg-green-100","dark:bg-green-900","text-green-800","dark:text-green-200"),t.classList.remove("bg-blue-100","dark:bg-blue-900","text-blue-800","dark:text-blue-200"))}function y(){const t=d.getStatistics(),e=document.getElementById("competitorCount");e&&(e.textContent=`${t.selectedCompetitorCount}/${t.competitorCount}`);const s=document.getElementById("semanticCount");s&&(s.textContent=`${t.selectedSemanticElements}/${t.totalSemanticElements}`);const o=document.getElementById("aspectCount");o&&(o.textContent=`${t.selectedAspects}/${t.totalAspects}`);const n=document.getElementById("statsCard");n&&t.competitorCount>0&&n.classList.remove("hidden");const i=document.getElementById("proceedBtn");if(i){const r=t.selectedCompetitorCount>0||t.selectedSemanticElements>0||t.selectedAspects>0;i.disabled=!r}}function ie(){const t=document.getElementById("selectAllBtn"),e=document.getElementById("deselectAllBtn"),s=document.getElementById("proceedBtn");t&&t.addEventListener("click",()=>{d.selectAllElements(),I(d.results)}),e&&e.addEventListener("click",()=>{d.deselectAllElements(),I(d.results)}),s&&s.addEventListener("click",()=>{const o=d.getSelectedData();console.log("Selected data for phase 2:",o),alert("Fase 2 non ancora implementata. I dati selezionati sono stati salvati nella console.")})}function re(){ae(),le(),de(),me(),ie()}function ae(){const t=document.getElementById("toggleOpenrouterApiKey");t&&t.addEventListener("click",()=>{ce("openrouterApiKey","openrouterEyeIcon")})}function ce(t,e){const s=document.getElementById(t),o=document.getElementById(e);if(!s||!o)return;const n=s.type==="password";s.type=n?"text":"password",o.innerHTML=n?'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21" />':'<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />'}function le(){["openrouterApiKey","topicInput"].forEach(e=>{const s=document.getElementById(e);s&&s.addEventListener("input",()=>{if(e.includes("ApiKey")){const o=e.replace("ApiKey","");O(o,s.value)}setTimeout(()=>$(),0)})})}function de(){const t=document.getElementById("generateBtn");t&&t.addEventListener("click",ue)}async function ue(){try{const t=document.getElementById("openrouterApiKey").value.trim(),e=document.getElementById("topicInput").value.trim();if(!$())return;if(V("Verifica API key..."),!await F(t)){A(),T("API key OpenRouter non valida. Verifica e riprova.");return}const o=await d.generateCompleteOutline(t,e);I(o),A(),W("Generazione completata con successo!")}catch(t){A(),T(`Errore durante la generazione: ${t.message}`),console.error("Generation error:",t)}}function me(){d.setProgressCallback((t,e)=>{pe(t,e)}),d.setStatusCallback(t=>{he(t)})}function pe(t,e){const s=document.getElementById("progressSection"),o=document.getElementById(`step${t}`);if(s&&s.classList.remove("hidden"),o){const n=o.querySelector(".w-4.h-4.rounded-full"),i=o.querySelector("span");n&&(n.className="w-4 h-4 rounded-full bg-blue-500"),i&&(i.textContent=e,i.className="text-sm font-medium text-blue-600 dark:text-blue-400");for(let r=1;r<t;r++){const a=document.getElementById(`step${r}`);if(a){const l=a.querySelector(".w-4.h-4.rounded-full");l&&(l.className="w-4 h-4 rounded-full bg-green-500");const c=a.querySelector("span");c&&(c.className="text-sm text-green-600 dark:text-green-400")}}}}function he(t){const e=document.getElementById("statusMessage");e&&(e.textContent=t)}function $(){const t=document.getElementById("openrouterApiKey").value.trim(),e=document.getElementById("topicInput").value.trim(),s=document.getElementById("generateBtn"),o=document.getElementById("generateHelp");let n=!0,i="";return t?e?e.length<3&&(n=!1,i="L'argomento deve essere di almeno 3 caratteri"):(n=!1,i="Inserisci un argomento/topic da analizzare"):(n=!1,i="Inserisci la tua API key di OpenRouter"),s&&(s.disabled=!n),o&&(i?(o.textContent=i,o.classList.remove("hidden")):o.classList.add("hidden")),n}document.addEventListener("DOMContentLoaded",()=>{N(),M(),re(),document.body.classList.add("loaded")});
